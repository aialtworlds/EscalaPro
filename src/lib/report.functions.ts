...
export const monthlyReport = createServerFn({ method: "GET" })
  ...
  .inputValidator((d: unknown) => z.object({ month: monthSchema }).parse(d))
  .handler(async ({ data, context }) => {
    ...
    const rows: EmployeeReport[] = employees.map((e) => {
      ...
      const worked = own.filter((s) => s.status !== "absent");
      ...
      for (const s of worked) {
        ...
        const res = evaluateShift(shift, rule, worked as unknown as RuleShift[], {
          holidays,
    ...
    const freelancerShifts = shifts.filter((s) => s.is_freelancer);
    ...
    return {
      ...
      totals: {
        ...
        freelancerShifts: freelancerShifts.length,
      },
    };
  });
