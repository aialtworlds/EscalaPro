  ...
  .inputValidator((d: unknown) =>
    z
      .object({
        ...
        sector_only: z.boolean().default(true),
      })
      .parse(d),
  )
...
/** Edição de um turno já salvo (nome, setor, dia, horários, pessoas). */
export const updateDemand = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await context.supabase
      .from("demand_templates")
      .update({ ...patch, label: patch.label?.trim() || null })
      .eq("id", id)
      .select()
      .single();
    if (error) throw new Error(error.message);
